...
import { NATURAL_PITCH_CLASS, pitchClassToNoteName } from './notes';
...
// (label, pc, tonicName) → the target's *display* name spelled by its degree, so the 7th
// of E major is D♯, not E♭. The degree number fixes the letter (tonic letter + number −
// 1); the pitch class fixes the accidental. `tonicName` is the *spelled* tonic
// (spelling.js's spellScaleTonic), so C♯ minor's degrees read from C, not D♭. See
// docs/architecture/music-theory.md's "Registers" note.
export function degreeNoteName(label, pc, tonicName) {
  const letter = LETTERS[(LETTERS.indexOf(tonicName[0]) + label.number - 1) % 7];
